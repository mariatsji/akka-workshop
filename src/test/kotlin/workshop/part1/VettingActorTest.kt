...
class VettingActorTest : AkkaTest() {
    ...
    fun respondsWithNumberOfVettedAds() {
        ...
        sender.send(vettingActor, GetNumVettedAds())
        assertThat(sender.expectMsgClass(Duration.Zero(), NumVettedAds::class.java).numVettedAds, equalTo(1))
    }
    ...
    fun reportsNumVettedAds() {
        ...
        assertThat(numVettedAdsActor.expectMsgClass(Duration.Zero(), NumVettedAds::class.java).numVettedAds, equalTo(0))

        sender.send(vettingActor, createAd())
        sender.send(vettingActor, ReportNumVettedAds())
        assertThat(numVettedAdsActor.expectMsgClass(Duration.Zero(), NumVettedAds::class.java).numVettedAds, equalTo(1))

        sender.send(vettingActor, createAd())
        sender.send(vettingActor, ReportNumVettedAds())
        assertThat(numVettedAdsActor.expectMsgClass(Duration.Zero(), NumVettedAds::class.java).numVettedAds, equalTo(2))
    }
